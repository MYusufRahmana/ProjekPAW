...
<div class="add-cinema-button">
    <a href="{{ route('pesanan.create') }}" class="btn btn-success mb-2 w-10"><i class="fa-solid fa-plus"></i> Add Pesanan</a>
</div>
            ...
            <tbody>
                @foreach($pesanan as $data)
                    @foreach($jadwal as $item)
                        @if($item->id == $data->id_jadwal)
                            <tr>
                                <td>{{$item->film->judul}}</td>
                                <td>{{$item->cinema->nama_ruangan}}</td>
                                <td>{{$item->cinema->kategori}}</td>
                                <td>{{$item->tanggal}}</td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <form method="POST" action="{{ route('pesanan.destroy', $data->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{ route('pesanan.edit', $data->id) }}" class="btn btn-xs btn-secondary btn-flat btn-edit">Edit</a>
